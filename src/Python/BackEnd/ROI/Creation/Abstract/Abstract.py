...
class CreateRoiAbstract(QLabel, Loger):
    ...
    def createAndAddROIToList(self, scatter=False):
        if self.__checIfSizeIsValid():
            roi = Point(self, self.x2, self.y2, self.roiNames + 1, self.mainWindow.manipulatorInterferes.x,
                        self.mainWindow.manipulatorInterferes.y, self.pixelAbsolutValue)
        else:
            roi = ROI(self, self.x1, self.y1, self.x2, self.y2, self.roiNames + 1,
                      self.mainWindow.manipulatorInterferes.x,
                      self.mainWindow.manipulatorInterferes.y, self.pixelAbsolutValue, scatter=scatter)

        self.mainWindow.addROIToList(roi)

    def createAndAddLineToList(self):
        roi = Line(self, self.x1, self.y1, self.x2, self.y2, self.roiNames + 1, self.mainWindow.manipulatorInterferes.x,
                   self.mainWindow.manipulatorInterferes.y, self.pixelAbsolutValue)
